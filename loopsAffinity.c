...
int main(int argc, char *argv[]) {
  ...
  //Variable values
  idleTimeBarrier = (double *) malloc(sizeof(double) * P);
  idleTimeLT = (double *) malloc(sizeof(double) * P);
  ...
  shared(idleTimeLT, idleTimeBarrier, lock_ut, thread_rng, is_assigned, default_ldd_tarr, getThread) private(r)
  {
  for (r=0; r<reps; r++){
  ...
  idletimeb = 0;
  idletimel = 0;
  //printf("idle time 1 = %d\n", idleTimeLT[0]);
  for (t = 0; t < P; t++){
    thread_rng[t] = 0;
    idletimeb += idleTimeBarrier[t];
    idletimel += idleTimeLT[t];
    idleTimeBarrier[t] = 0;
    idleTimeLT[t] = 0;
  }
  printf("Loop 1: Average idle time at load transfer = %f , Average idle time at barrier = %f\n",idletimel/(double) P, idletimeb/(double) P);
  init2();
  start2 = omp_get_wtime();
  #pragma omp parallel default(none) \
  shared(P, tf, seg_sz,seg_szP, itrLeftArr,d_seg_sz, localSetEnd) \
  shared(idleTimeBarrier, idleTimeLT, ut, lock_ut, thread_rng, is_assigned, default_ldd_tarr, getThread) private(r)
  {
  for (r=0; r<reps; r++){
    loop2();
  }
  ...
  printf("Loop 2: Average idle time at load transfer = %f , Average idle time at barrier = %f\n",idletimel/(double) P, idletimeb/(double) P);
  //debug_race();
  free(d_seg_sz);
  free(itrLeftArr);
  ...
  */
  ...
  while (max_val > 0){
   ...
   // Load transfer block //
    ...
    {
    tend = omp_get_wtime();
    idleTimeLT[t_no] += tend - tstart;
    //#pragma omp critical (printf_lock)
    //{printf("Time spent at critical = %f\n",tend=tstart);}
    for (t = 0; t < P; t++){
  ...
  #pragma omp barrier
  tend = omp_get_wtime();
  idleTimeBarrier[t_no] += tend - tstart;
}
